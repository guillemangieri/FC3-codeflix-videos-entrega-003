...
import * as CastMemberSequelize from '../cast-members-sequelize';
...
describe('CastMemberModelMapper Integration Tests', () => {
  ...
  it('should throws error when cast member is invalid', () => {
    const model = CastMemberModel.build({
      ...
      name: 'a'.repeat(256), // inválido para a regra de domínio
      type: 999 as any, // inválido para CastMemberType
      created_at: new Date(),
    });
    try {
      ...
      expect((e as LoadEntityError).error).toMatchObject([
        ...
        { type: ['Invalid cast member type: 999'] },
      ]);
    }
  });
